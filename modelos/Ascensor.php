...
class Ascensor
{
	...
	public function informacionFMMantencion($idsap, $fm)
	{
		...
		foreach ($rsptaJson->value as $val) {
			$listado[] = array(
				"ServiceCallID" => isset($val->ServiceCalls->ServiceCallID) ? $val->ServiceCalls->ServiceCallID : 0,
				"CustomerName" => isset($val->ServiceCalls->CustomerName) ? $val->ServiceCalls->CustomerName : '',
				"Subject" => isset($val->ServiceCalls->Subject) ? $val->ServiceCalls->Subject : '',
				"CustomerCode" => isset($val->ServiceCalls->CustomerCode) ? $val->ServiceCalls->CustomerCode : '',
				"InternalSerialNum" => isset($val->ServiceCalls->InternalSerialNum) ? $val->ServiceCalls->InternalSerialNum : '',
				"BuildingFloorRoom" => isset($val->CustomerEquipmentCards->U_NX_NCC) ? $val->CustomerEquipmentCards->U_NX_NCC : '',
				...
				"U_NX_MODELO" => isset($val->Items->U_NX_MODELO) ? $val->Items->U_NX_MODELO : '',
				"U_NX_NOMENCLATURACL" => isset($val->Items->U_NX_NOMENCLATURACL) ? $val->Items->U_NX_NOMENCLATURACL : ''
			);
		}
		return $listado;
	}
